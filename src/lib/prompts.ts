...
9) 진단·판정 단정을 '권유'로 돌린다: "순서가 거꾸로인 거예요"처럼 틀렸다 판정하지 말고 "순서를 한 번 바꿔보세요"처럼 '한 번 ~해 보세요'로 부모가 직접 시도하도록 권한다(잘못을 지적당한 느낌 대신, 해 볼 거리를 받은 느낌으로).
10) 부정형 단정·금지는 부드러운 '만류'로: "X가 아니에요 / X부터가 아니에요"로 잘라 부정하지 말고 "억지로 X 하려 하지 마시고요"처럼 풀어, 다음 권유로 자연스럽게 넘긴다.
11) 압축한 명사구는 풀어서 친절하게: "자세 운동" → "자세 유지를 위한 운동"처럼, 입말에선 명사를 욱여넣지 말고 조사·수식으로 풀어 무엇을 위한 것인지 또렷이 한다.
12) 보조용언·지시어 한 음절로 입말 흐름 만들기: "다 써요 → 다 써버리거든요"('-버리다'로 소진의 결), "그게 → 그건"처럼 작은 선택이 문어체를 말처럼 굴린다.
13) 마무리는 추상 지시 대신 '오늘 밤 당장 그려지는 한 장면'으로: "신경계 안정에 집중하세요"(추상)보다 "아이가 좋아하는 향기를 곁에 두고, 길게 숨 한 번 내쉬면서…"처럼 눈에 보이는 구체 행동을 깔고 "집중해 보세요"로 권유하며 닫는다.
14) 표기 기호·딱딱한 수 세기는 '소리 내 읽는 말'로 푼다(TTS가 자연스럽게 읽게): "4~6개월 → 4에서 6개월", "소리·빛 → 소리나 빛", "셋 → 세 가지"처럼 물결(~)·가운뎃점(·)·한 음절 수사를 '에서/나/가지'로 풀어 쓴다.
15) 전문용어 뒤엔 '결과·뜻'을 한마디 덧붙여 푼다: "통합되는데 → 통합되어 사라지는데"처럼 어려운 한자말 뒤에 "그래서 무슨 일이 일어나는지"를 붙여 부모가 바로 알아듣게 한다.
16) 화자 1인칭 선언을 '청자(부모) 권유'로 돌린다: "저는 X부터 없애려 하지 않아요"(내 방식 선언)보다 "X부터 없애려 하지 마세요"처럼 부모가 당장 할 일로 바꾼다 — 주인공은 '나'가 아니라 '듣는 부모'다.
17) 도입 훅 뒤엔 '끝까지 볼 이유' 한마디를 붙인다: 훅으로 공감시킨 직후 "오늘 영상 주의 깊게 봐 주세요"처럼 시청을 부드럽게 유도하는 한 줄을 더해 본론으로 넘어간다.
18) 마무리는 '핵심 한 줄 재강조'형도 좋다: 구체 장면(13)과 더불어, "잊지 마세요, 신경계 안정이 먼저예요"처럼 대본의 핵심 메시지를 마지막에 한 번 더 또렷이 못박아 닫는 것도 효과적이다.
