...
1) 문맥·논리 연결: 컷을 1번부터 순서대로 소리 내어 읽었다고 상상한다. 앞 문장이 꺼낸 내용에 뒷 문장이 자연스럽게 이어지는가? 앞에서 말하지 않은 전제를 갑자기 단정하지 않는가?
   · 나쁜 예) "긴장을 풀어주는 가장 짧은 길은 호흡과 향기예요." → 바로 다음에 "안전감 없이 호흡부터 시키면 더 긴장해요." (앞에서 '호흡부터 시키는 상황'을 꺼낸 적이 없는데 갑자기 반박·경고가 나와 비약·모순이 생김.)
   · 고치는 법) 두 문장 사이에 연결 다리를 놓거나(예: "단, 순서가 중요해요. 안전감이 먼저예요."), 순서를 바꾸거나, 한쪽을 덜어낸다. 비약·모순이 0이 될 때까지 다듬는다.
